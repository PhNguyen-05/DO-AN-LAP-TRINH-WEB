...
<form:form id="${param.formId}" modelAttribute="promotion" method="post"
	...
	<div class="mb-3">
		<label for="promotionName" class="form-label fw-bold">Tên
			Khuyến Mãi <span class="text-danger">*</span>
		</label>
		<form:input path="promotionName" id="promotionName"
			class="form-control" placeholder="Nhập tên khuyến mãi"
			required="true" />
	...
	<div class="mb-3">
		...
		<form:textarea path="description" id="description"
			class="form-control" rows="3" placeholder="Nhập mô tả khuyến mãi" />
		<form:errors path="description" cssClass="text-danger" />
	</div>

	<div class="mb-3">
		<label for="discountValue" class="form-label fw-bold">Giá Trị
			Giảm <span class="text-danger">*</span>
		</label>
		<form:input path="discountValue" id="discountValue"
			class="form-control" type="number" min="0" step="0.01"
			value="<fmt:formatNumber value='${promotion.discountValue}' maxFractionDigits='0'/>"
			placeholder="Nhập giá trị giảm" required="true" />

		<form:errors path="discountValue" cssClass="text-danger" />
	</div>

	<div class="mb-3">
		<label for="discountType" class="form-label fw-bold">Loại Giảm
			<span class="text-danger">*</span>
		</label>
		<form:select path="discountType" id="discountType" class="form-select"
			required="true">
	...
	<div class="mb-3">
		<label for="vendorId" class="form-label fw-bold">Nhà Cung Cấp
			<span class="text-danger">*</span>
		</label>
		<form:select path="vendor.id" id="vendorId" class="form-select"
			required="true">
					...
					<h2 class="accordion-header" id="heading-${category.id}">
						<button
							...
							<input type="checkbox" name="categoryIds" value="${category.id}"
								...
								id="cat-${category.id}" data-category-id="${category.id}" /> <label
								for="cat-${category.id}" class="form-check-label fw-bold mb-0">${category.name}</label>
						</button>
					</h2>
