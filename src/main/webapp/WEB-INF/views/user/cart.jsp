...
<sitemesh:page title="Giỏ hàng - StarShop">
	<sitemesh:head>
		<link
			href="https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600&family=Dancing+Script:wght@600&display=swap"
			rel="stylesheet">
		<link
...
.btn-outline-pink:hover {
	...
	<div class="cart-container">
		...
		<c:if test="${empty cart.cartItems}">
			<div class="text-center py-5">
				...
				<a href="${pageContext.request.contextPath}/shop"
					class="btn btn-outline-pink">Tiếp tục mua sắm</a>
			</div>
		</c:if>
		...
		<c:if test="${not empty cart.cartItems}">
			...
			<form id="cartForm" method="get"
				action="${pageContext.request.contextPath}/user/cart/delete-selected">
				<div class="d-flex align-items-center mb-3">
					<input type="checkbox" id="selectAll" class="me-2"> <label
						for="selectAll" class="fw-medium me-auto">Chọn tất cả</label>
				...
				<c:forEach var="item" items="${cart.cartItems}">
					<div class="cart-item">
						<input type="checkbox" name="selectedItems" value="${item.id}"
							class="item-checkbox me-3"> <img
							src="${pageContext.request.contextPath}/images/${item.product.imageUrl}"
							alt="${item.product.name}">
						<div class="item-info">
							<a
								href="${pageContext.request.contextPath}/product/${item.product.id}"
								class="item-name">${item.product.name}</a><br> <span
								class="text-muted small">${item.product.sku}</span><br> <span
								class="item-price" data-price="${item.unitPrice}"> <fmt:formatNumber
									value="${item.unitPrice}" type="currency" currencySymbol="₫" />
							</span>
						</div>
						...
						<div class="ms-3">
							<strong class="item-total text-danger"> <fmt:formatNumber
									value="${item.unitPrice * item.quantity}" type="currency"
									currencySymbol="₫" />
							</strong>
						</div>
					</div>
			...
			<div class="total-bar">
				<div>
					<strong>Tổng cộng:</strong> <span id="cartTotal"
						class="text-danger fw-bold"> <fmt:formatNumber
							value="${cart.totalAmount}" type="currency" currencySymbol="₫" />
					</span>
				</div>
	...
	<script>
				...
				.on(
						...
						function() {
							...
							if (confirm('Bạn có chắc muốn xóa các sản phẩm đã chọn?')) {
								window.location.href = '${pageContext.request.contextPath}/user/cart/delete-selected?ids='
										+ ids.join(',');
							}
						});
			...
			$('.cart-item').each(
					function() {
						if ($(this).find('.item-checkbox').is(':checked')) {
							let qty = parseInt($(this).find('.quantity-input')
									.val());
							let price = parseFloat($(this).find('.item-price')
									.data('price'));
							total += qty * price;
						}
					});
		...
		// Khi nhấn "Đặt hàng", gửi danh sách sản phẩm được chọn
		$('#checkoutForm').on('submit', function(e) {
			...
			// Nối danh sách ID sản phẩm vào form
			$('#selectedItemsInput').val(selected.join(','));
			this.submit();
		});
	</script>
=======
