...
public class AdminProductController {
	...
	public String listProducts(Model model, @RequestParam(defaultValue = "0") int page,
			@RequestParam(defaultValue = "10") int size, @RequestParam(required = false, defaultValue = "") String name,
	...
	public String viewProductDetail(@PathVariable("id") Integer id, Model model) {
		...
		if (product.getCreatedAt() != null) {
			java.util.Date createdAtDate = java.util.Date
					.from(product.getCreatedAt().atZone(java.time.ZoneId.systemDefault()).toInstant());
    ...
    public String addProduct(
            // (Toàn bộ @RequestParam như ở trên)
            ...
            @RequestParam("vendor.id") Integer vendorId) throws IOException {
        ...
        // Lấy Category (code này đã có trong ví dụ trước)
        Category category = categoryService.findById(categoryId)
                .orElseThrow(() -> new RuntimeException("Category not found"));
        product.setCategory(category);

        // ⭐ 1. THÊM LOGIC GÁN SKU (SỬA LỖI UNIQUE KEY)
        product.setSku(sku); 

        // ⭐ 2. THÊM LOGIC GÁN VENDOR (SỬA LỖI VENDOR NULL)
        Vendor vendor = vendorService.findById(vendorId) // Giả sử bạn có hàm findById trong VendorService
                .orElseThrow(() -> new RuntimeException("Vendor not found"));
        product.setVendor(vendor);

        // Xử lý upload ảnh (Dùng code cũ của bạn)
